...
def load_data():
    ...
    data = ConllCorpusReader(r'C:\Users\shazi\OneDrive - vit.ac.in\sem7\slp\da2', ['eng.train'],
...
def train(request):
    ...
    # Add CRF model to the session dict for use in result()
    crf_pk = jsonpickle.encode(crf)
    request.session['model'] = crf_pk

    # Returns predict.html
    return predict(request)


def result(request):
    # Get CRF model from session dict for prediction
    crf_pk = request.session.get('model')
    crf = jsonpickle.decode(crf_pk)

    # Obtains user-input text from predict.html
    ...
    # Append the named entities in the sentence to res list
    res = []
    for word, label in zip(step1, predicted_labels):
        res.append(f"Word: {word}, Predicted Label: {label}")

    # Sends res to result.html and returns the web page
